...
is_sourced() {
    # 被 source 时，$0 是当前 shell（如 bash），BASH_SOURCE[0] 是脚本文件
    [[ "$0" != "${BASH_SOURCE[0]}" ]]
}
...
write_theme_block() {
    ...
    case "$theme" in
        1)
            # 方案 1：当前的双行 Hacker 风（红用户名 + 蓝主机 + 黄路径 + 绿框）
            cat << 'EOF' >> "$BASHRC"
...
EOF
            ;;
        4)
            # 方案 4：带时间的信息风（时间 + 用户名@主机 + 路径）
            cat << 'EOF' >> "$BASHRC"

# === Hacker style prompt & ls colors (added by irismt) ===

# 方案 4：信息风（时间 + 用户 + 主机 + 路径）
...
# 应用主题：备份 + 清理旧块 + 写入新主题 + 尝试刷新
apply_theme() {
    echo "== 选择美化方案 =="

    echo "  1) 方案 1：双行 Hacker 风（红用户名 + 蓝主机 + 黄路径 + 绿框）"
    echo "  2) 方案 2：单行彩色箭头风（root@iris → /path #）"
    echo "  3) 方案 3：极简 Matrix 绿风（全绿）"
    echo "  4) 方案 4：带时间的信息风（[12:34] root@iris:/path#）"
    echo "  0) 取消，返回主菜单"
    echo

    local t
    while true; do
        read -rp "请输入主题编号（0/1/2/3/4）： " t
        [[ "$t" =~ ^[0-5]$ ]] || { echo "❌ 请输入 0~4 的数字。"; continue; }
        break
    done
    ...
    if is_sourced; then
        ...
        echo "🎉 美化已完成。"
        echo "⚠ 由于你是通过 'bash irismt.sh' 方式运行脚本，脚本在子 shell 中执行，"
        echo "   无法自动刷新你当前这个终端的环境。"
        echo "👉 请在当前终端手动执行："
        echo "   source \"$BASHRC\""
